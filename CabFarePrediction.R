...
# We cant use mode method becoz data will be more biased towards passenger_count = 1
...
# We will do outlier analysis only on Fare_amount just for now and we do outlier analysis after feature engineering latitudes and longitudes
# Boxplot for fare_Amount
pl1 = ggplot(train, aes(x = factor(passenger_count), y = fare_amount))
pl1 + geom_boxplot(outlier.colour = "red", fill = "grey", outlier.shape = 18)+ylim(0,100)

# Replace all the outliers with NA and impute
...
# new features will be yea, month, day_of_week, hour
# Convert pickup_datetime from factor to datetime
...
sum(is.na(train)) # There was 1 na in pickup_datetime
train = na.omit(train) # we will remove that 1 row of na
...
cnames = colnames(numeric_index)
# Correlation analysis for numeric variables
corrgram(train[,numeric_index],upper.panel = panel.pie, main = "Correlation Plot")

# ANOVA for categorical variables with target numeric variables
...
###################################  Finalizing and saving the model for later use #######################################
# In this step we will train the model on whole training dataset and save that model for later use
